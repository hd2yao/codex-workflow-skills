...
核心原则：先用现有摘要和账本做轻量发现，再由用户确认，最后交给 `skill-governance-review` 决定沉淀形态。
...
3. 对候选打分：出现次数、来源文件数、是否跨项目、是否已有替代能力、是否可能节省 token。
4. 输出候选报告，分为：
   - 推荐沉淀
   - 建议合并或归档
   - 暂不处理
5. 对任何“创建/更新/归档/安装 skill”的建议，必须再使用 `skill-governance-review`。
...
- 下一步只给可确认动作，例如“确认 P0 候选后再进入 skill-governance-review”。
